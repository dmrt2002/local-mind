# LocalMind Documentation

Welcome to the LocalMind documentation! This directory contains all project documentation organized by category.

## 📁 Documentation Structure

### `/setup` - Getting Started
- **QUICK_START.md** - Quick start guide for new users
- **MODEL_SETUP.md** - Model configuration and setup
- **EMBEDDING_MODEL_SETUP.md** - Embedding model configuration
- **ICONS_SETUP.md** - Application icons setup

### `/development` - Development Guides
- **LOGGING_GUIDE.md** - Logging system usage
- **MIGRATIONS.md** - Database migration guide
- **DATABASE_CONNECTION.md** - Database connection configuration
- **README_DEBUGGING.md** - Debugging tips and tricks
- **OPEN_CONSOLE.md** - Console access for debugging

### `/features` - Feature Documentation
- **SEARCH_FEATURES.md** - Search functionality overview
- **SEARCH_TECHNIQUES.md** - Advanced search techniques

### `/archive` - Historical Documentation
Contains outdated status reports and implementation notes for reference.

## 📚 Main Documentation Files

- **TECHNICAL_DOCUMENTATION.md** - Complete technical reference
- **USER_FLOW_DOCUMENTATION.md** - User interface and flow documentation
- **TESTING_GUIDE.md** - Testing procedures and guidelines
- **FEATURE_ROADMAP.md** - Feature planning and implementation roadmap

## 🚀 Quick Links

- [Quick Start](setup/QUICK_START.md) - Get started in 5 minutes
- [Technical Docs](TECHNICAL_DOCUMENTATION.md) - Full technical reference
- [Testing Guide](TESTING_GUIDE.md) - How to test the application
- [Feature Roadmap](FEATURE_ROADMAP.md) - Feature planning and priorities

## 📖 For New Developers

1. Start with [QUICK_START.md](setup/QUICK_START.md)
2. Review [TECHNICAL_DOCUMENTATION.md](TECHNICAL_DOCUMENTATION.md)
3. Check [development/](development/) for dev-specific guides
4. See [TESTING_GUIDE.md](TESTING_GUIDE.md) for testing practices

## 🗺️ For Feature Planning

1. Review [FEATURE_ROADMAP.md](FEATURE_ROADMAP.md) for prioritized features
2. Check [USER_FLOW_DOCUMENTATION.md](USER_FLOW_DOCUMENTATION.md) for pain points
3. See current implementation phases and success metrics
